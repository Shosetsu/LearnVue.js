...
const updateHistory = ()=>{
	...
	href = href.substring(href.lastIndexOf(projectName)).replace(projectName,"..");
	data.unshift(href);
	...
	navi_vm = new Vue({
		...
		components: {
			'navi-bar': {
				...
				<button onclick='openLink(\"../index.html\",null,false)'>Back to List</button>
				<button v-for='bean in history' :onclick='"openLink(\\\""+bean.link+"\\\",null,false)"'>{{bean.label}}</button>
